...
// example invokation (darwin): gofmt -w `find . -name \*.go` && (cd server_standalone/ ; go build -tags debug) && go test -tags debug github.com/mazzegi/sftp -integration -v -sftp /usr/libexec/sftp-server -run ServerCompareSubsystems
